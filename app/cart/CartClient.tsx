...
export default function CartClient() {
  ...
  const handleCheckout = async () => {
    ...
    toast.info("Checkout flow will be integrated with Razorpay soon!");
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 flex flex-col gap-4">
                {cartItems.map((item) => (
                  <div key={item.automation.id} className="flex flex-col sm:flex-row items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-2xl">
                    <div className="relative w-full sm:w-32 aspect-video rounded-lg overflow-hidden bg-black/40">
                      {item.automation.thumbnailUrl && (
                        <Image src={item.automation.thumbnailUrl} alt={item.automation.title} fill className="object-cover" />
                      )}
                    </div>
                    <div className="flex-1 flex flex-col">
                      <Link href={`/automations/${item.automation.slug}`} className="text-lg font-bold text-white hover:text-primary-fixed-dim transition-colors mb-1">
                        {item.automation.title}
                      </Link>
                      <span className="text-primary-fixed-dim font-bold">₹{(item.automation.price / 100).toLocaleString()}</span>
                    </div>
                    <button 
