...
export default function CartClient() {
  ...
  const total = cartItems.reduce(
    (acc, item) => acc + (item.plan ? item.plan.price : item.automation.price),
    0
  );
      ...
      <div className="bg-background min-h-screen flex flex-col">
        ...
        <main className="flex-1 pt-32 pb-20 px-6 max-w-[1200px] mx-auto w-full">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20 bg-white/80 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 flex flex-col gap-4">
                {cartItems.map((item) => {
                  const itemPrice = item.plan ? item.plan.price : item.automation.price;
                  return (
                    <div
                      key={item.automation.id}
                      className="flex flex-col sm:flex-row items-center gap-4 p-4 bg-white/80 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl shadow-sm dark:shadow-none"
                    >
                      <div className="relative w-full sm:w-32 aspect-video rounded-lg overflow-hidden bg-black/10 dark:bg-black/40">
                      ...
                      <div className="flex-1 flex flex-col">
                        <Link
                          ...
                          className="text-lg font-bold text-slate-900 dark:text-white hover:text-primary-fixed-dim transition-colors mb-1"
                        >
                          {item.automation.title}
                        </Link>
                        {item.plan && (
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs px-2 py-0.5 rounded bg-primary/20 text-cyan-800 dark:text-primary font-mono font-bold">
                              {item.plan.name}
                            </span>
                            {item.plan.maintenanceEnabled && (
                              <span className="text-[11px] text-cyan-700 dark:text-cyan-300">
                                (+₹{(item.plan.maintenancePrice / 100).toLocaleString()}/mo maint.)
                              </span>
                            )}
                          </div>
                        )}
                        <span className="text-primary-fixed-dim font-bold">
                          ₹{(itemPrice / 100).toLocaleString()}
                        </span>
                      </div>
                      <button
                        onClick={() => handleRemove(item.automation.id)}
                        className="p-3 text-red-500 hover:text-red-600 hover:bg-red-400/10 rounded-lg transition-colors cursor-pointer"
                        title="Remove"
                      >
              ...
              {/* Order Summary */}
              <div className="w-full lg:w-80 h-fit p-6 bg-white/80 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl flex flex-col shadow-sm dark:shadow-none">
                ...
                <div className="border-t border-black/10 dark:border-white/10 pt-4 mb-6 flex justify-between items-center text-lg font-bold text-slate-900 dark:text-white">
                  <span>Total</span>
