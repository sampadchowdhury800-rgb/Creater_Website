...
interface WorkspaceClientProps {
  userAutomation: {
    ...
    automation: {
      ...
      executionMode: "MANUAL" | "EVENT_DRIVEN";
      hasWorkflow?: boolean;
...
export default function WorkspaceClient({ userAutomation }: WorkspaceClientProps) {
  ...
  const [workspaceStatus, setWorkspaceStatus] = useState(userAutomation.status);
  const [integrationsSatisfied, setIntegrationsSatisfied] = useState(
    userAutomation.status === "ACTIVE"
  );

  const handleIntegrationStatusChange = useCallback((allSatisfied: boolean) => {
    setIntegrationsSatisfied(allSatisfied);
    if (allSatisfied) {
      setWorkspaceStatus((prev) => (prev === "NOT_CONFIGURED" ? "ACTIVE" : prev));
    } else {
      setWorkspaceStatus("NOT_CONFIGURED");
    }
  }, []);

  const auto = userAutomation.automation;
  const isEventDriven = auto.executionMode === "EVENT_DRIVEN";
  ...
  return (
    <div className="bg-background text-on-surface min-h-screen flex flex-col font-sans selection:bg-cyan-500/20 selection:text-cyan-200 transition-colors">
      ...
      <main className="flex-1 pt-28 pb-20 px-4 sm:px-6 max-w-7xl mx-auto w-full">
        ...
        <div className="bg-white dark:bg-surface-container-low border border-slate-200 dark:border-white/10 rounded-3xl p-6 sm:p-8 shadow-xs mb-8 transition-colors">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                {!isEngineConfigured ? (
                  ...
                  <span
                    ...
                    className="px-2.5 py-0.5 bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30 text-xs font-bold rounded-md flex items-center gap-1"
                  >
                  ...
                  <span
                    id="workspace-status-badge"
                    className="px-2.5 py-0.5 bg-cyan-50 dark:bg-cyan-950/40 text-cyan-700 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-500/30 text-xs font-bold rounded-md flex items-center gap-1"
                  >
                  ...
                  <span
                    id="workspace-status-badge"
                    className="px-2.5 py-0.5 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30 text-xs font-bold rounded-md flex items-center gap-1"
                  >
            ...
            <div className="flex flex-col sm:flex-row gap-2 shrink-0">
              ...
              {/* Run button — only shown for MANUAL automations with engine configured and active workspace */}
              {!isEventDriven && isEngineConfigured && auto.isExecutable && userAutomation.status !== "DISABLED" && userAutomation.status !== "PAUSED" && (
                <button
              ...
              {/* Event-driven status badge — shown instead of Run button for EVENT_DRIVEN automations */}
              {isEventDriven && (
                <span
                  id="event-driven-status-badge"
                  className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-cyan-950/40 text-blue-700 dark:text-cyan-300 border border-blue-200 dark:border-cyan-500/30 font-semibold text-xs rounded-xl"
                  title="This automation runs automatically when incoming Gmail emails are detected. Manual execution is not supported."
                >
                  <Zap className="w-3.5 h-3.5" />
                  Runs Automatically
                </span>
        ...
        {/* Event-Driven Automation Banner */}
        {isEventDriven && (
          <div className="mb-6 p-5 bg-cyan-500/10 border border-cyan-500/30 rounded-2xl flex items-start gap-4 text-cyan-900 dark:text-cyan-200">
            <Zap className="w-6 h-6 text-cyan-600 dark:text-cyan-400 shrink-0 mt-0.5" />
            <div>
              <h4 className="font-bold text-sm text-cyan-950 dark:text-cyan-100 mb-1">
                Event-Driven Automation
              </h4>
              <p className="text-xs text-cyan-800 dark:text-cyan-300 leading-relaxed">
                This automation runs <strong>automatically</strong> whenever new Gmail support emails arrive in your connected inbox.
                There is no manual &ldquo;Run&rdquo; button — execution is triggered by incoming email events.
                Connect your Gmail account below and new emails will be processed automatically.
              </p>
            </div>
          </div>
        )}
            ...
            {loadingExecs ? (
              ...
              <div className="bg-white dark:bg-surface-container-low border border-slate-200 dark:border-white/10 rounded-2xl p-8 text-center shadow-xs transition-colors">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-xs max-w-md mx-auto">
                   {isEventDriven
                    ? "This automation runs automatically when new Gmail support emails are detected. Executions will appear here as emails arrive."
                    : isExecutionAllowed
          ...
          <div className="bg-white dark:bg-surface-container-low border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-xs space-y-4 transition-colors">
            ...
            <div className="p-4 bg-slate-50 dark:bg-surface-container border border-slate-200 dark:border-white/10 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <span
                ...
                className={`px-2.5 py-1 text-xs font-bold rounded-md w-fit ${
