      ...
      <div className="bg-background selection:bg-primary-container selection:text-on-primary-container min-h-screen flex flex-col">
        ...
        <main className="flex-1 pt-32 pb-24 px-4 sm:px-6 max-w-[1200px] mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div>
              <div className="flex items-center gap-2 text-cyan-700 dark:text-primary text-sm font-label-caps uppercase tracking-widest mb-3">
          ...
          {posts.length === 0 ? (
            <div className="text-center py-20 bg-white/80 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl">
              <Sparkles className="w-12 h-12 text-primary-fixed-dim mx-auto mb-4" />
              <p className="text-lg text-slate-900 dark:text-white font-bold mb-2">No news published yet</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {posts.map((post) => {
                ...
                return (
                  <article
                    key={post.id}
                    className="bg-white/80 dark:bg-white/5 border border-black/10 dark:border-white/10 hover:border-primary/40 rounded-2xl overflow-hidden shadow-sm dark:shadow-none transition-all duration-300 flex flex-col group"
                  >
                    {/* Image / Thumbnail */}
                    <div className="relative aspect-video w-full bg-black/10 dark:bg-black/40 overflow-hidden">
                      {post.featuredImage ? (
                        <Image
                      ...
                      <div>
                        <div className="flex items-center gap-3 text-xs text-on-surface-variant mb-3">
                          <span className="flex items-center gap-1">
                            <Calendar className="w-3.5 h-3.5 text-cyan-700 dark:text-primary" />
                            {new Date(post.publishDate || post.createdAt).toLocaleDateString()}
                          </span>
                          {post.readingTime && (
                            <span className="flex items-center gap-1">
                              <Clock className="w-3.5 h-3.5 text-cyan-700 dark:text-primary" />
