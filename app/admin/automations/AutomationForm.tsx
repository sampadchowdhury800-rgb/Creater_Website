...
import { ArrowLeft, Save, Loader2, Image as ImageIcon, Video, Trash2, FileText, UploadCloud, Download, Sparkles } from "lucide-react";
import Link from "next/link";
import Image from "next/image";
import FaqEditor, { FaqItem } from "@/components/editor/FaqEditor";
import { ConfigSchemaEditor } from "./ConfigSchemaEditor";
import { IntegrationRequirementsEditor } from "./IntegrationRequirementsEditor";
...
export default function AutomationForm({ initialData, categories }: AutomationFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    // Execution engine binding — admin-only fields
    n8nWorkflowId: initialData?.n8nWorkflowId || "",
    isExecutable: initialData?.isExecutable !== false ? "true" : "false",
  });
  ...
  const [configSchema, setConfigSchema] = useState<ConfigSchema>(
    (initialData?.configSchema as ConfigSchema) || { fields: [] }
  );

  const [integrationRequirements, setIntegrationRequirements] = useState<AutomationIntegrationRequirements>(
    (initialData?.integrationRequirements as AutomationIntegrationRequirements) || { requirements: [] }
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      payload.append("configSchema", JSON.stringify(configSchema));
      payload.append("integrationRequirements", JSON.stringify(integrationRequirements));
      if (isNew) {
      ...
      // Re-throw Next.js redirect/not-found signals — NEVER swallow these.
      // redirect() in a Server Action throws NEXT_REDIRECT internally; if caught
      // here it causes React Error #441 in production (uncaught server exception).
      if (
    ...
    <form onSubmit={handleSubmit} className="p-6 md:p-8 max-w-5xl mx-auto space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* User Configuration Interface (configSchema) */}
          <ConfigSchemaEditor value={configSchema} onChange={setConfigSchema} />

          {/* Third-Party OAuth Integration Requirements */}
          <div className="bg-[#1C1C1E] border border-white/10 rounded-2xl p-6">
            <IntegrationRequirementsEditor
              value={integrationRequirements}
              onChange={setIntegrationRequirements}
            />
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-[#1C1C1E] border border-white/10 rounded-2xl p-6 space-y-4">
            ...
            {initialData?.id && (
              <div className="pt-3 border-t border-white/10 space-y-2">
                <Link
                  href={`/admin/automations/${initialData.id}/plans`}
                  className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-primary/10 border border-primary/30 text-primary hover:bg-primary/20 text-sm font-bold transition-all cursor-pointer shadow-sm"
                >
                  <Sparkles size={16} />
                  <span>Configure Commercial Plans</span>
                </Link>
                <p className="text-[11px] text-on-surface-variant text-center">
                  Configure custom pricing tiers (Trials, 1m, 6m, Lifetime, Maintenance).
                </p>
              </div>
            )}
          </div>
          ...
          {/* Execution Engine Binding — Admin Only */}
          <div className="bg-[#1a1200] border border-yellow-700/40 rounded-2xl p-6 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-lg font-bold text-yellow-300">Execution Engine</h2>
              <span className="text-[10px] font-semibold text-yellow-500 bg-yellow-950/60 border border-yellow-700/30 px-2 py-0.5 rounded-full">Admin Only</span>
            </div>
            <p className="text-xs text-yellow-500/70">
              The n8n Workflow ID is a server-side reference only. It is never exposed to users or the browser bundle.
              Only set this once the n8n workflow exists and is ready for execution.
            </p>

            <div>
              <label className="block text-sm font-medium text-yellow-200 mb-2">n8n Workflow ID</label>
              <input
                type="text"
                name="n8nWorkflowId"
                value={formData.n8nWorkflowId}
                onChange={handleChange}
                placeholder="Leave empty if workflow not yet configured"
                className="w-full bg-white/5 border border-yellow-700/30 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-400 text-sm"
              />
              <p className="text-[11px] text-yellow-600 mt-1.5">
                This is the internal workflow identifier in your n8n instance. Do not expose actual API keys here.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-1">
              <input
                ...
                id="isExecutable"
                name="isExecutable"
                checked={formData.isExecutable === "true"}
                onChange={(e) =>
                  setFormData((prev) => ({ ...prev, isExecutable: e.target.checked ? "true" : "false" }))
                }
                className="w-5 h-5 accent-yellow-400 rounded cursor-pointer"
              />
              <label htmlFor="isExecutable" className="text-sm font-medium text-yellow-200 cursor-pointer">
                Allow users to execute this automation
              </label>
            </div>
          </div>
        </div>
