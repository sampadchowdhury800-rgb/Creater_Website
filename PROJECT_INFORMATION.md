...
| **Google APIs** | Native REST Client | Native `fetch` | OAuth 2.0 PKCE, Gmail REST API, Google Cloud Pub/Sub |
...
│   │   └── webhooks/
│   │       ├── gmail/                 # Google Cloud Pub/Sub push notification receiver
...
[Google Cloud Pub/Sub Push Notification]
                  │
                  ▼
[POST /api/webhooks/gmail] ◄── Verify Pub/Sub token
                  │
                  ├─► 1. Lookup GmailWatchSubscription (Resolve tenant & userAutomationId)
                  ├─► 2. Check Entitlement (Active license/trial via checker.ts)
                  ├─► 3. Atomic Idempotency (Check ProcessedGmailEvent by historyId)
                  │
                  ▼
[lib/integrations/grant-service.ts] (Mints 5-minute single-use Execution Grant: SUPPORT_AI)
                  │
                  ▼
...
3. **Gmail Inbound Push Pipeline:**
   - `GmailWatchSubscription`: Subscriptions active with Google Cloud Pub/Sub, expiration dates, synchronization `historyId`.
   - `ProcessedGmailEvent`: Atomic deduplication ledger indexed by `[emailAddress, historyId]`.
...
- **Provider API Client:** `lib/integrations/providers/google.ts`.
  ...
  - `executeGmailWatch`: Subscribes to Google Cloud Pub/Sub push notifications.
  - `executeGmailStopWatch`: Cancels active push notifications.
  - `executeGmailListHistory`: Synchronizes message history events since a `startHistoryId`.
- **Watch Management Service:** `lib/integrations/watch-service.ts`.
- **Inbound Pub/Sub Webhook:** `app/api/webhooks/gmail/route.ts`.
- **Watch Renewal Cron:** `app/api/cron/renew-watches/route.ts`.
...
|---|---|---|---|---|---|
| `POST` | `/api/webhooks/gmail` | Ingests Google Pub/Sub push notifications | Pub/Sub Token | `emailAddress` ──► `GmailWatchSubscription` | `IMPLEMENTED` |
| `POST` | `/api/integrations/google/watch` | Subscribes connected mailbox to watch | Clerk | `getCurrentUserId()` | `IMPLEMENTED` |
| `DELETE`| `/api/integrations/google/watch` | Unsubscribes mailbox from watch | Clerk | `getCurrentUserId()` | `IMPLEMENTED` |
| `GET`  | `/api/cron/renew-watches` | Proactively renews expiring watches | Bearer `CRON_SECRET` | System-wide batch | `IMPLEMENTED` |
...
| `app/api/webhooks/gmail/route.ts` | Webhook | Ingests Google Cloud Pub/Sub push notifications |
| `app/api/cron/renew-watches/route.ts` | Maintenance | Cron job renewing 7-day Gmail watches |
...
| **Gmail Push Watch Service**| `IMPLEMENTED` | `lib/integrations/watch-service.ts` | Watch registration & renewal active |
| **Gmail Pub/Sub Webhook** | `IMPLEMENTED` | `app/api/webhooks/gmail/route.ts` | Inbound receiver with idempotency ledger |
| **Watch Renewal Cron** | `IMPLEMENTED` | `app/api/cron/renew-watches/route.ts`| Secured by `CRON_SECRET` |
